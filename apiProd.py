...
# Config de cookies (ajustável entre localhost e produção)
if "localhost" in os.environ.get("RENDER_EXTERNAL_HOSTNAME", ""):
    # Ambiente local
    app.config.update(
...
def api_server():
    ...
    try:
        ...
        return jsonify({"message": "Erro no servidor, tente mais tarde", "status": 500}), 500
...
def update_products(product_id, product_name=None, product_image=None, product_price=None):
    try:
        ...
        if product_price:
            cursor.execute('UPDATE produto SET preco_prod = %s WHERE id_prod = %s;', (product_price, product_id))
